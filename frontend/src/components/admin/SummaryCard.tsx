...
export default function SummaryCard({ title, value, icon: Icon, delay = 0 }: SummaryCardProps) {
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className="bg-white rounded-[2rem] p-6 shadow-sm hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 relative overflow-hidden group border border-gray-100"
    >
      ...
      <div>
        ...
        <p className="text-3xl font-bold text-gray-900 group-hover:bg-brand-gradient group-hover:text-transparent group-hover:bg-clip-text transition-all duration-300">
          {value}
        </p>
      </div>
    </motion.div>
