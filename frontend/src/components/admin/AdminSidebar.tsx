...
export default function AdminSidebar({ activeItem, onNavigate, collapsed = false, onToggleCollapse }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-t border-gray-100 dark:border-gray-800 relative transition-colors duration-300" ref={dropdownRef}>
        <AnimatePresence>
          {isDropdownOpen && (
            <motion.div 
            ...
            >
              <div className="flex flex-col gap-1">
                <button onClick={() => { setIsDropdownOpen(false); navigate('/profile'); }} className={`flex items-center ${collapsed ? 'justify-center p-3' : 'gap-3 p-2'} text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-gray-800 rounded-xl transition-colors text-sm font-medium w-full text-left`} title="Profile">
                  <User size={18} />
                  {!collapsed && <span>Profile</span>}
                </button>
                <button onClick={() => { setIsDropdownOpen(false); navigate('/'); }} className={`flex items-center ${collapsed ? 'justify-center p-3' : 'gap-3 p-2'} text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-gray-800 rounded-xl transition-colors text-sm font-medium w-full text-left`} title="Home">
                  <Home size={18} />
                  {!collapsed && <span>Home</span>}
                </button>
                <div className="h-px bg-gray-100 dark:bg-gray-800 my-1 w-full transition-colors duration-300" />
                <button onClick={() => { setIsDropdownOpen(false); handleLogout(); }} className={`flex items-center ${collapsed ? 'justify-center p-3' : 'gap-3 p-2'} text-gray-600 dark:text-gray-300 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-gray-800 rounded-xl transition-colors text-sm font-medium w-full text-left`} title="Logout">
                  <LogOut size={18} />
        ...
        >
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0 text-left">
              <p className="text-sm font-semibold text-gray-900 dark:text-white truncate group-hover:text-blue-600 transition-colors duration-300">{userName}</p>
