        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50/50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800 transition-colors">
              {columns.map((col) => (
                <th key={col.key} className="px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {col.header}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800 transition-colors">
            {filteredData.length > 0 ? (
                  ...
                  {(onEdit || onDelete || onView) && (
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        {onView && (
                          <button onClick={() => onView(row)} className="text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-gray-700 p-1.5 rounded-lg transition-colors" title="View">
                            <Eye size={16} />
                          </button>
                        )}
                        {onEdit && (
                          <button onClick={() => onEdit(row)} className="text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-gray-700 p-1.5 rounded-lg transition-colors" title="Edit">
                            <Edit3 size={16} />
                          </button>
                        )}
                        {onDelete && (
                          <button onClick={() => onDelete(row)} className="text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-gray-700 p-1.5 rounded-lg transition-colors" title="Delete">
                            <Trash2 size={16} />
                          </button>
                        )}
                      </div>
              ...
              <tr>
                <td colSpan={columns.length + 1} className="px-6 py-8 text-center text-gray-500 dark:text-gray-400 text-sm">
                  No data found
                </td>
              </tr>
            )}
